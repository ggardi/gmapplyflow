...
export default function ApplyForm({ flash }) {
    ...
    const mockLoanOfficers = [
        ...
        { id: 4, name: 'Matthew McConaughey', branch: 'Main Street Branch', state: 'TX' },
        { id: 5, name: 'Jennifer Lopez', branch: 'Central Branch', state: 'TX' },
        { id: 6, name: 'Pete Davidson', branch: 'South Branch', state: 'TX' },
        { id: 7, name: 'Jimmie Walker', branch: 'East Side Branch', state: 'NY' },
        { id: 8, name: 'Elton John', branch: 'Broadway Branch', state: 'NY' },
        { id: 9, name: 'John Doe', branch: 'Financial District Branch', state: 'NY' }
    ];
    ...
    const handleLoanOfficerSearch = (searchTerm) => {
        ...
        const filtered = mockLoanOfficers.filter(officer => 
    ...
    const handleSubmit = (e) => {
        ...
        const isLoanOfficerValid = validateField('loan_officer', data.loan_officer);
        const isPasswordValid = validateField('password', data.password);
        const isPasswordConfirmationValid = validateField('password_confirmation', data.password_confirmation);
        ...
        <div className="form-container">
            ...
            {showRestorePrompt && savedDataToRestore && (
                <div className="alert alert-info" style={{ marginBottom: 'var(--space-6)' }}>
                    <div style={{ marginBottom: 'var(--space-3)' }}>
                        <strong>📋 We found your previous application data!</strong>
                    </div>
                    <div style={{ marginBottom: 'var(--space-4)', fontSize: 'var(--font-size-sm)' }}>
                        Last saved: {formatSavedDataTime(savedDataToRestore.timestamp)}
                    </div>
            ...
            <form className="form" onSubmit={handleSubmit} noValidate>
                ...
                {emailExists && emailChecked && (
                    <div className="alert alert-warning">
                        ...
                        <a href="/login" className="link">
                            Click here to login
                        </a> or use a different email address to continue.
                    </div>
                )}

                {!emailExists && emailChecked && EMAIL_REGEX.test(data.email) && (
                ...
                <SearchableSelect
                    ...
                    placeholder={data.state ? "Search loan officers or branches..." : "Please select a state first"}
